...
def test_get_autoscaling_group(group):
...
def test_increase_capacity(group):
...
def test_list_scheduled_activities(group):
	activities = autoscaling.list_scheduled_activities(group)
	assert activities is not None

def test_list_scaling_activities(group):
...
# Helpers
def wait_for_capacity_change(group, expected):
	sleep_time=5
	max_attempts=10
	attempt=0
